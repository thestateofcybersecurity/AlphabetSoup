...
- **The 2026 Security Leadership Scorecard** (`/tools/`): ten demands that repeat across roughly 60 live security leadership job postings (manager through SVP/CISO), ranked by how often they appear. Each card gives the demand in plain English, composite examples of how job descriptions phrase it, why organizations pay for it, and the on-the-job tool that proves the skill, with live tools linked as they ship. The dataset (`src/data/scorecard.json`) is the single source of truth the tools are built against.
