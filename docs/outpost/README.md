...
| [mockups/screens.html](mockups/screens.html) | Interactive mockup of all seven screens in the Outpost theme: Command Deck, Bridge, Hangar, mission Briefing (score, unanswered evidence, delta, recommendations with mode ceilings), Sortie detail (phases, preview, execute, rollback calls, verification, live stream), remediation Plan (capacity, quarter board, KPIs), and Perimeter (verified assets, ownership challenge, diff). | Rendered headlessly in dark and light modes and at phone width; layout bugs found on the first look were fixed. |
| [connectors/README.md](connectors/README.md) | The connector contract: lifecycle, capability grammar, secret record, health, scope enforcement, evidence and check shapes, write-call rules, failure taxonomy. | Review. |
| [connectors/entra-id.md](connectors/entra-id.md), [connectors/aws.md](connectors/aws.md) | The first two connector specs: auth model, capability to permission mapping, trust instructions and templates, health check, scope model, missions with every check, playbooks with preview, verify, and rollback, failure modes. | Review against vendor permission references at build time. |
| [connectors/seed.sql](connectors/seed.sql), [connectors/check_catalog.sql](connectors/check_catalog.sql) | Catalog rows for both connectors (2 types, 6 missions, 45 checks, 12 playbooks, 3 agents, 2 MCP servers) and a thirteen-rule consistency lint. | Loads on the schema, idempotent; lint returns nothing on the seed and flags five deliberately injected bugs. |
| [`../outpost/`](../outpost/) | The repository skeleton, laid out as a standalone project: migrations from schema.sql plus a user-role column, the catalog seed, both SQL lints, the vendored engine contract, the tenant middleware (token, session revocation, `tenant_id` rejection, `SET LOCAL` scoping), session switch and return with dual-chain audit, the policy table, connector request and customer-only completion with sealed trust material, the catalog with computed unlocks, flight recorder reads, a CLI, and the two-tenant test harness. | ruff clean; nine tests pass against a fresh Postgres 16 database, including an OpenAPI walk that hits every id-bearing route cross-tenant and every route with a `tenant_id`; CLI migrate, seed, and lint smoke-tested. |
...
1. Create the `outpost` GitHub repository and move the skeleton there with `git subtree split --prefix=outpost`; wire CI to run `make lint test` against a Postgres service.
2. Hand ENGINE-CONTRACT.md and engine_contract.py to the engine team as the 1.0 retrofit scope; replace the vendored `contract.py` when the package ships.
3. Verify every Graph permission and IAM action in the connector specs against the current vendor references, then implement the Entra ID and AWS drivers behind `connectors_driver.Driver`.
4. Replace `DevEnvelope` with the KMS envelope and OIDC login for `dev-login`; both are the gate for the first real customer.
5. Missions, findings, and the Temporal sortie workflow, in that order, each with its own two-tenant tests.
