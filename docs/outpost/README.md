...
1. Hand ENGINE-CONTRACT.md and engine_contract.py to the engine team as the 1.0 retrofit scope.
2. Verify every Graph permission and IAM action in the connector specs against the current vendor references, and pin the CloudFormation quick-create link.
3. Stand up the Outpost repository skeleton: FastAPI app with the tenant middleware, the two-tenant test harness, migrations from schema.sql, and the catalog loader from seed.sql.
