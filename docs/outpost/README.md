...
| [mockups/screens.html](mockups/screens.html) | Interactive mockup of the Command Deck, Bridge, and Hangar in the Outpost theme, with the dual-approval flow and the connector trust step. | Rendered headlessly in dark and light modes and at phone width; two grid bugs found and fixed. |
...
3. Remaining screens: the mission Briefing, sortie detail with execute and rollback phases, the remediation plan, and the scanner asset verification flow.
4. Stand up the Outpost repository skeleton: FastAPI app with the tenant middleware, the two-tenant test harness, migrations from schema.sql, and the catalog loader from seed.sql.
