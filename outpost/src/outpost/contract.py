...
class MissionRepository(Protocol):
    ...
    async def list(
        self, session: TenantSession, *, tool: str | None = None, limit: int = 50, cursor: str | None = None
    ) -> tuple[Sequence[Mission], str | None]: ...
...
class KnowledgeStore(Protocol):
    async def search(
        self, ctx: RequestContext, resolver: IndexResolver, query: str, *, corpora: Sequence[Corpus] = ("shared", "tenant"), k: int = 8
